...
| [**sample-nodejs-gitops**](https://github.com/1bugo2/sample-nodejs-gitops) | what is deployed and where: ArgoCD `Application`s and per-environment values |

Why they are separate is explained under [Decisions](#decisions-and-why).

Claims in this document were verified against a running cluster, and the captured output is
in [`docs/evidence/`](docs/evidence/). Caveats and known limitations are stated rather than
omitted.
...
The chart targets a specific stack rather than every possible cluster. Production
infrastructure is built for a known platform, and each portability flag adds a code path that
nothing exercises.
...
- `SampleNodejsHighErrorRate` queries `traefik_service_requests_total`. Behind nginx that
  metric does not exist, so the alert never fires at all. A silent alert is worse than a
  broken one. Swap it for the equivalent nginx metric.
- Two dashboard panels (request rate, latency percentiles) read the same Traefik histograms
...
default to `false`. The reason is bootstrap ordering, not portability.
...
This is the reason to use GitOps rather than an artifact of the lab setup. The
...
None of it is needed at runtime, since dependencies are resolved in the build stage, so the
runtime stage deletes `npm`, `corepack` and `yarn`. Trivy then exits 0 on HIGH/CRITICAL. The
CVEs are removed rather than suppressed in a `.trivyignore`, and a package manager is a useful
thing for an attacker to find in a container.
...
That is bootstrap ordering rather than a hedge against unknown clusters. The app chart legitimately
...
its own evidence is unavailable while you are triaging the failure.
...
4. TRIVY GATE ◄────────── the reason for this ordering
...
### Six alerts, and why event-loop lag matters most
...
Final state: only `Watchdog` firing, which is expected, since it is a dead-man's switch
...
A StatefulSet would make this deployment slower and more fragile in exchange for
...
The brief offers a choice between a dedicated GitOps repo and ArgoCD reading the app repo
directly, and asks which and why. This uses a separate repo, for four reasons:

**1. CI never holds a cluster credential.** The pipeline's whole deploy
...
The costs: two repos to keep in step, and a deploy is not atomic with its merge, since there is
a gap between merging and ArgoCD syncing. Both are acceptable. The credential separation is not
something a single repo can give you.
...
Self-management through the app-of-apps is a common pattern, but ArgoCD is installed here by
[`scripts/bootstrap-vm.sh`](https://github.com/1bugo2/sample-nodejs-gitops/blob/main/scripts/bootstrap-vm.sh)
in the GitOps repo.

If ArgoCD breaks itself, it can no longer be used to repair itself, so a script is the more
reliable recovery path. The ArgoCD instance already present on this VM before the exercise was
self-managed and was sitting in `Unknown` sync state when I found it. The app-of-apps manages
the `AppProject`, the chart repository registration and the environment `Application`s.
...
The least predictable of the seven. PID 1 gets no default signal handling: the kernel only
...
**No security headers, and no WAF.** Both are deliberate rather than oversights. The app sends no
...
[`docs/evidence/`](docs/evidence/) contains 15 screenshots and 8 captured-output documents,
indexed with what each one shows.
