...
| [12-ingress-prod-my-app.png](screenshots/12-ingress-prod-my-app.png) | `Hello, World!` from the **prod** Ingress at `my-app-prod.192-168-56-101.nip.io/my-app`. Prod genuinely serves, it is not just an Application object |
| [13-prod-outofsync-healthy.png](screenshots/13-prod-outofsync-healthy.png) | **The gate at rest.** dev `Healthy/Synced` at `1.2.1`; prod `Healthy` but **`OutOfSync`**, still on `1.2.0`. `Healthy` rather than `Missing` is the important part. Prod is serving normally with a change pending, which is what a gated environment looks like. Sidebar counts confirm it: Synced 2, OutOfSync 1, Degraded 0 |
| [14-prod-pending-diff.png](screenshots/14-prod-pending-diff.png) | **What a human actually approves.** ArgoCD's DIFF view for prod, showing 10 resources OutOfSync. The pane is scrolled to the ConfigMap, where the chart version label moves `1.2.0` → `1.2.1`; the Deployment's diff in the same view carries the image digest change. The point is that approval is a reviewable diff, not an unexplained button |
| [15-prod-deploy-policy-info.png](screenshots/15-prod-deploy-policy-info.png) | The `spec.info` block on the prod Application, stating the policy in the UI: *"Manual sync required (promotion gate). OutOfSync means a release awaits approval, not a fault."* Added because `OutOfSync` reads as a fault to anyone who does not speak ArgoCD fluently, and the gate should not rely on noticing an *absent* `syncPolicy.automated` block |
## Captured output
