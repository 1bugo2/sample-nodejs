...
| [10-argocd-hpa-scaled-to-6.png](screenshots/10-argocd-hpa-scaled-to-6.png) | **The HPA scaled to 6 replicas under load**, captured in ArgoCD. Five pods are 4 minutes old against one at an hour, and the Application stayed `Healthy`/`Sync OK` throughout � which is the `ignoreDifferences: /spec/replicas` setting working, otherwise `selfHeal` would have reverted the autoscaler on the next sync |
